...
(() => {
  ...
  if (exhibit) {
    ...
    function reveal(phase, count) {
      ...
      stages.forEach(
        (stage, index) => (stage.dataset.shown = String(index < count)),
      );
    }
    function play() {
      ...
      function frame(now) {
        ...
        if (elapsed < 1500) {
          ...
          nodes.forEach(
            (item) =>
              (item.node.textContent = item.text.slice(
                0,
                Math.max(0, characters - item.start),
              )),
          );
        } else {
          answer.innerHTML = original;
  ...
  function validate() {
    ...
    rules.forEach(([input, valid, message]) => {
      ...
      form.querySelector(`[data-err-for="${input.id}"]`).textContent = valid
        ? ""
        : message;
    });
  ...
  async function submit(event) {
    ...
    try {
      ...
      } else status.append("We will send the link by email.");
      status.append(
        " Nothing enters a truth registry until a person approves it.",
      );
    } catch {
      failure("That did not reach us, so treat it as unsent.");
    } finally {
      pending = false;
