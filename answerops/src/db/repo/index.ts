import { statements, tenantRecord, atomic, groupRows } from './statements.js';
/**
 * Repository layer.
 *
...
/** Compatibility row for the legacy public repository API; concrete foundational entities
 * above and snapshot/role records have typed results. Domain payloads remain open records. */
export interface Row {
  [k: string]: any;
}

// ---------------------------------------------------------------- tenants/users
export function createTenant(db: DB, name: string, plan = 'operate'): TenantRow {
  const t = { id: id('ten'), name, plan, created_at: nowIso() };
  statements(db)
    .prepare('INSERT INTO tenants (id, name, plan, created_at) VALUES (@id, @name, @plan, @created_at)')
    .run(t);
  return t;
}
...
export function listTenants(db: DB): TenantRow[] {
  return statements(db).prepare('SELECT * FROM tenants ORDER BY created_at').all() as TenantRow[];
}

export function getTenant(db: DB, tenantId: string): TenantRow | undefined {
  return statements(db).prepare('SELECT * FROM tenants WHERE id = ?').get(tenantId) as TenantRow | undefined;
}
...
export function findUserByEmail(db: DB, email: string): UserRow | undefined {
  return statements(db).prepare('SELECT * FROM users WHERE email = ?').get(email.toLowerCase()) as
    | UserRow
    | undefined;
}
...
export function createSession(
  ...
  statements(db)
    .prepare(
      'INSERT INTO sessions (id, tenant_id, user_id, csrf, created_at, expires_at, active_brand_id) VALUES (@id, @tenant_id, @user_id, @csrf, @created_at, @expires_at, @active_brand_id)',
    )
    .run(session);
  return session;
}
...
export function getSession(
  db: DB,
  sessionId: string,
): (SessionRow & Pick<UserRow, 'email' | 'role'>) | undefined {
  const session = statements(db)
    .prepare(
      'SELECT s.*, u.email, u.role FROM sessions s JOIN users u ON u.id = s.user_id AND u.tenant_id = s.tenant_id WHERE s.id = ?',
    )
    .get(sessionId) as (SessionRow & Pick<UserRow, 'email' | 'role'>) | undefined;
...
export function deleteSession(db: DB, sessionId: string): void {
  statements(db).prepare('DELETE FROM sessions WHERE id = ?').run(sessionId);
}

// ---------------------------------------------------------------------- brands
export function createBrand(db: DB, tenantId: string, name: string, domain: string, category = ''): BrandRow {
  const b = tenantRecord(tenantId, {
    id: id('brd'),
    tenant_id: tenantId,
    name,
    domain,
  ...
  statements(db)
    .prepare(
      'INSERT INTO brands (id, tenant_id, name, domain, category, created_at) VALUES (@id, @tenant_id, @name, @domain, @category, @created_at)',
    )
    .run(b);
  return b;
}

export function listBrands(db: DB, tenantId: string): BrandRow[] {
  return statements(db)
    .prepare('SELECT * FROM brands WHERE tenant_id = ? ORDER BY created_at')
    .all(tenantId) as BrandRow[];
}

export function getBrand(db: DB, tenantId: string, brandId: string): BrandRow | undefined {
  return statements(db)
    .prepare('SELECT * FROM brands WHERE tenant_id = ? AND id = ?')
    .get(tenantId, brandId) as BrandRow | undefined;
}

export function primaryBrand(db: DB, tenantId: string): BrandRow | undefined {
  return statements(db)
    .prepare('SELECT * FROM brands WHERE tenant_id = ? ORDER BY created_at LIMIT 1')
    .get(tenantId) as BrandRow | undefined;
}
...
export function listDemandSignals(db: DB, tenantId: string, brandId: string): Row[] {
  return statements(db)
    .prepare('SELECT * FROM demand_signals WHERE tenant_id = ? AND brand_id = ? ORDER BY volume DESC')
    .all(tenantId, brandId) as Row[];
}

export function attachSignalsToCluster(
  db: DB,
  tenantId: string,
  clusterId: string,
  signalIds: string[],
  ...
  atomic(db, () => {
    const update = statements(db).prepare(
      'UPDATE demand_signals SET cluster_id = ? WHERE tenant_id = ? AND id = ?',
    );
    signalIds.forEach((signalId) => update.run(clusterId, tenantId, signalId));
  });
}
...
export function setClusterMarkets(
  db: DB,
  tenantId: string,
  clusterId: string,
  geos: string[],
  ...
  statements(db)
    .prepare('UPDATE intent_clusters SET geos = ?, languages = ? WHERE tenant_id = ? AND id = ?')
    .run(
      JSON.stringify(geos.length ? geos : ['US']),
      JSON.stringify(languages.length ? languages : ['en']),
      tenantId,
      clusterId,
    );
}
...
export function listClusters(db: DB, tenantId: string, brandId: string): Row[] {
  return statements(db)
    .prepare(
      'SELECT * FROM intent_clusters WHERE tenant_id = ? AND brand_id = ? ORDER BY demand_weight DESC, label',
    )
    .all(tenantId, brandId) as Row[];
}
...
export function getCluster(db: DB, tenantId: string, clusterId: string): Row | undefined {
  return statements(db)
    .prepare('SELECT * FROM intent_clusters WHERE tenant_id = ? AND id = ?')
    .get(tenantId, clusterId) as Row | undefined;
}

export function updateClusterWeights(
  db: DB,
  tenantId: string,
  clusterId: string,
  weight: number,
  ...
  statements(db)
    .prepare('UPDATE intent_clusters SET demand_weight = ?, demand_volume = ? WHERE tenant_id = ? AND id = ?')
    .run(weight, volume, tenantId, clusterId);
}
...
export function createPromptVariant(
  ...
  statements(db)
    .prepare(
      'INSERT INTO prompt_variants (id, tenant_id, cluster_id, prompt, geo, language, created_at) VALUES (@id, @tenant_id, @cluster_id, @prompt, @geo, @language, @created_at)',
    )
    .run(row);
  return row;
}
...
export function listVariants(db: DB, tenantId: string, clusterId: string): Row[] {
  return statements(db)
    .prepare('SELECT * FROM prompt_variants WHERE tenant_id = ? AND cluster_id = ? ORDER BY created_at')
    .all(tenantId, clusterId) as Row[];
}
...
export function createTruthSource(db: DB, tenantId: string, brandId: string, s: Row): Row {
  ...
  statements(db)
    .prepare(
      'INSERT INTO truth_sources (id, tenant_id, brand_id, title, url, source_class, published_at, created_at) VALUES (@id, @tenant_id, @brand_id, @title, @url, @source_class, @published_at, @created_at)',
    )
    .run(row);
  return row;
}
...
export function listTruthSources(db: DB, tenantId: string, brandId: string): Row[] {
  return statements(db)
    .prepare('SELECT * FROM truth_sources WHERE tenant_id = ? AND brand_id = ? ORDER BY created_at')
    .all(tenantId, brandId) as Row[];
}
...
export function listCanonicalClaims(db: DB, tenantId: string, brandId: string): Row[] {
  return statements(db)
    .prepare(
      'SELECT * FROM canonical_claims WHERE tenant_id = ? AND brand_id = ? ORDER BY subject, predicate, effective_from DESC',
    )
    .all(tenantId, brandId) as Row[];
}
...
export function getCanonicalClaim(db: DB, tenantId: string, claimId: string): Row | undefined {
  return statements(db)
    .prepare('SELECT * FROM canonical_claims WHERE tenant_id = ? AND id = ?')
    .get(tenantId, claimId) as Row | undefined;
}

export function approveClaim(db: DB, tenantId: string, claimId: string, approver: string): void {
  statements(db)
    .prepare('UPDATE canonical_claims SET approved_by = ?, approved_at = ? WHERE tenant_id = ? AND id = ?')
    .run(approver, nowIso(), tenantId, claimId);
}
...
export function supersedeClaim(
  ...
  statements(db)
    .prepare(
      'UPDATE canonical_claims SET effective_to = ?, superseded_by_id = ? WHERE tenant_id = ? AND id = ?',
    )
    .run(effectiveTo, newClaimId, tenantId, oldClaimId);
}

// ------------------------------------------------------------------ model runs
export function insertRun(db: DB, tenantId: string, r: Row): Row {
  const row = tenantRecord(tenantId, { id: id('run'), tenant_id: tenantId, created_at: nowIso(), ...r });
  statements(db)
    .prepare(
...
export function listRuns(db: DB, tenantId: string, brandId: string, limit = 100): Row[] {
  return statements(db)
    .prepare(
      'SELECT * FROM model_runs WHERE tenant_id = ? AND brand_id = ? ORDER BY requested_at DESC LIMIT ?',
    )
...
export function getRun(db: DB, tenantId: string, runId: string): Row | undefined {
  return statements(db)
    .prepare('SELECT * FROM model_runs WHERE tenant_id = ? AND id = ?')
...
export function runsForCluster(db: DB, tenantId: string, clusterId: string, windowLabel?: string): Row[] {
  if (windowLabel) {
    return statements(db)
      .prepare(
        'SELECT * FROM model_runs WHERE tenant_id = ? AND cluster_id = ? AND window_label = ? ORDER BY requested_at',
      )
      .all(tenantId, clusterId, windowLabel) as Row[];
  }
  return statements(db)
    .prepare('SELECT * FROM model_runs WHERE tenant_id = ? AND cluster_id = ? ORDER BY requested_at')
    .all(tenantId, clusterId) as Row[];
}
...
export function observedForRun(db: DB, tenantId: string, runId: string): Row[] {
  return statements(db)
    .prepare('SELECT * FROM observed_claims WHERE tenant_id = ? AND run_id = ? ORDER BY created_at')
...
export function getCitation(db: DB, tenantId: string, citationId: string): Row | undefined {
  return statements(db)
    .prepare('SELECT * FROM citations WHERE tenant_id = ? AND id = ?')
    .get(tenantId, citationId) as Row | undefined;
}
 ...
 */
export function observedForWindow(
  db: DB,
  tenantId: string,
  brandId: string,
  windowLabel: string,
  ...
  const rows = statements(db)
    .prepare(
      'SELECT o.* FROM observed_claims o JOIN model_runs r ON r.id = o.run_id AND r.tenant_id = o.tenant_id WHERE o.tenant_id = ? AND r.brand_id = ? AND r.window_label = ? ORDER BY o.created_at',
    )
    .all(tenantId, brandId, windowLabel) as Row[];
  return groupRows(rows, (row) => row.run_id);
}
...
export function runsForWindow(db: DB, tenantId: string, brandId: string, windowLabel: string): Row[] {
  return statements(db)
    .prepare(
      'SELECT * FROM model_runs WHERE tenant_id = ? AND brand_id = ? AND window_label = ? ORDER BY created_at',
    )
    .all(tenantId, brandId, windowLabel) as Row[];
}
...
export function citationsForRun(db: DB, tenantId: string, runId: string): Row[] {
  return statements(db)
    .prepare('SELECT * FROM citations WHERE tenant_id = ? AND run_id = ? ORDER BY created_at')
    .all(tenantId, runId) as Row[];
}
...
export function runsWithMisconception(
  db: DB,
  tenantId: string,
  brandId: string,
...
export function listActions(db: DB, tenantId: string, brandId: string): Row[] {
  return statements(db)
    .prepare(
      'SELECT * FROM actions WHERE tenant_id = ? AND brand_id = ? ORDER BY priority DESC, created_at DESC',
    )
    .all(tenantId, brandId) as Row[];
}
...
export function getAction(db: DB, tenantId: string, actionId: string): Row | undefined {
  return statements(db)
    .prepare('SELECT * FROM actions WHERE tenant_id = ? AND id = ?')
    .get(tenantId, actionId) as Row | undefined;
}

export function setActionState(db: DB, tenantId: string, actionId: string, state: string): void {
  statements(db)
    .prepare('UPDATE actions SET state = ?, updated_at = ? WHERE tenant_id = ? AND id = ?')
    .run(state, nowIso(), tenantId, actionId);
}

export function setActionExperiment(db: DB, tenantId: string, actionId: string, experimentId: string): void {
  statements(db)
    .prepare('UPDATE actions SET experiment_id = ?, updated_at = ? WHERE tenant_id = ? AND id = ?')
    .run(experimentId, nowIso(), tenantId, actionId);
}
...
export function insertTransition(
  ...
  statements(db)
    .prepare(
      'INSERT INTO action_transitions (id, tenant_id, action_id, from_state, to_state, actor, note, created_at) VALUES (@id, @tenant_id, @action_id, @from_state, @to_state, @actor, @note, @created_at)',
    )
    .run(row);
  return row;
}
...
export function listTransitions(db: DB, tenantId: string, actionId: string): Row[] {
  return statements(db)
    .prepare('SELECT * FROM action_transitions WHERE tenant_id = ? AND action_id = ? ORDER BY created_at')
    .all(tenantId, actionId) as Row[];
}
...
export function getExperiment(db: DB, tenantId: string, experimentId: string): Row | undefined {
  return statements(db)
    .prepare('SELECT * FROM experiments WHERE tenant_id = ? AND id = ?')
    .get(tenantId, experimentId) as Row | undefined;
}

export function listExperiments(db: DB, tenantId: string, brandId: string): Row[] {
  return statements(db)
    .prepare('SELECT * FROM experiments WHERE tenant_id = ? AND brand_id = ? ORDER BY created_at DESC')
    .all(tenantId, brandId) as Row[];
}
...
export function insertBusinessOutcome(db: DB, tenantId: string, o: Row): Row {
  const row = tenantRecord(tenantId, { id: id('out'), tenant_id: tenantId, created_at: nowIso(), ...o });
  statements(db)
    .prepare(
...
export function outcomesForExperiment(db: DB, tenantId: string, experimentId: string): Row[] {
  return statements(db)
    .prepare('SELECT * FROM business_outcomes WHERE tenant_id = ? AND experiment_id = ? ORDER BY created_at')
    .all(tenantId, experimentId) as Row[];
}

// ------------------------------------------------------------------- crawlers
export function insertCrawlerEvent(db: DB, tenantId: string, e: Row): Row {
  const row = tenantRecord(tenantId, { id: id('crw'), tenant_id: tenantId, created_at: nowIso(), ...e });
  statements(db)
    .prepare(
...
export function listCrawlerEvents(db: DB, tenantId: string, brandId: string, limit = 500): Row[] {
  return statements(db)
    .prepare(
      'SELECT * FROM crawler_events WHERE tenant_id = ? AND brand_id = ? ORDER BY occurred_at DESC LIMIT ?',
    )
    .all(tenantId, brandId, limit) as Row[];
}
...
export function upsertEntity(
  ...
  return atomic(db, () => {
    const existing = statements(db)
      .prepare('SELECT * FROM entities WHERE tenant_id = ? AND name = ?')
      .get(tenantId, name) as Row | undefined;
    if (existing) return existing;
    const entity = tenantRecord(tenantId, { id: id('ent'), name, kind, domain, created_at: nowIso() });
    statements(db)
      .prepare(
        'INSERT INTO entities (id, tenant_id, name, kind, domain, created_at) VALUES (@id, @tenant_id, @name, @kind, @domain, @created_at)',
      )
      .run(entity);
    return entity;
  });
...
export function upsertRelationship(
  ...
  return atomic(db, () => {
    ...
    const relationship = tenantRecord(tenantId, {
      id: existing?.id ?? id('rel'),
    ...
    statements(db)
      .prepare(
        'INSERT INTO entity_relationships (id, tenant_id, brand_id, entity_id, relation, basis, confidence, note, created_at) VALUES (@id, @tenant_id, @brand_id, @entity_id, @relation, @basis, @confidence, @note, @created_at) ON CONFLICT(tenant_id, brand_id, entity_id) DO UPDATE SET relation = excluded.relation, basis = excluded.basis, confidence = excluded.confidence, note = excluded.note',
      )
      .run(relationship);
...
export function insertAlert(db: DB, tenantId: string, a: Row): Row {
  const row = tenantRecord(tenantId, { id: id('alt'), tenant_id: tenantId, created_at: nowIso(), ...a });
  statements(db)
    .prepare(
      'INSERT INTO alerts (id, tenant_id, brand_id, kind, headline, detail, p_value, effect, q_value, created_at) VALUES (@id, @tenant_id, @brand_id, @kind, @headline, @detail, @p_value, @effect, @q_value, @created_at)',
    )
    .run(row);
  return row;
}
...
export function listAlerts(db: DB, tenantId: string, brandId: string): Row[] {
  return statements(db)
    .prepare('SELECT * FROM alerts WHERE tenant_id = ? AND brand_id = ? ORDER BY created_at DESC')
    .all(tenantId, brandId) as Row[];
}
...
export function audit(
  ...
  statements(db)
    .prepare(
      'INSERT INTO audit_log (id, tenant_id, actor, action, target_type, target_id, summary, created_at) VALUES (@id, @tenant_id, @actor, @action, @target_type, @target_id, @summary, @created_at)',
    )
    .run({
      id: id('aud'),
      tenant_id: tenantId,
