...
export interface DefectItem {
  ...
  baselineComparison: { pValue: number; significant: boolean; qValue: number | null } | null;
}
...
function runsInClusters(db: DB, tenantId: string, brandId: string, clusterIds: string[], windowLabel: string): repo.Row[] {
...
export function buildDashboard(db: DB, tenantId: string, brandId: string, windowOverride?: string | null): DashboardData {
  ...
  const allRuns = db
    .prepare('SELECT * FROM model_runs WHERE tenant_id = ? AND brand_id = ? AND window_label = ?')
    .all(tenantId, brandId, current) as repo.Row[];
  const simulatedRuns = allRuns.filter((r) => r.simulated === 1).length;

  // ---------------------------------------------------------------- section 1
  const rollup = repo.misconceptionRollup(db, tenantId, brandId, current);
  const rawDefects: DefectItem[] = [];
  const pValues: number[] = [];

  for (const row of rollup) {
    const clusterIds = String(row.clusters ?? '').split(',').filter(Boolean);
    const denomRuns = runsInClusters(db, tenantId, brandId, clusterIds, current);
    const k = Number(row.defect_runs ?? 0);
    const n = denomRuns.length;
    const m = measure(k, n);
    const cluster = clusterById.get(clusterIds[0]) ?? clusters[0];
    if (!cluster) continue;

    const canonical = row.canonical_claim_id ? repo.getCanonicalClaim(db, tenantId, row.canonical_claim_id) : undefined;
    ...
    if (baseline) {
      const baseRuns = runsInClusters(db, tenantId, brandId, clusterIds, baseline);
      const baseK = repo.runsWithMisconception(db, tenantId, brandId, row.misconception_key, baseline).length;
      const t = twoProportionTest(baseK, baseRuns.length, k, n);
      comparison = { pValue: t.pValue, significant: t.significant, qValue: null };
    ...
    rawDefects.push({
      ...
      headline: buildDefectHeadline(providers, row.verdict, m, predicateOf(row.misconception_key)),
      verdict: row.verdict,
      severity: row.severity,
      exampleStatement: row.example_statement,
      measurement: m,
  ...
  for (const c of clusters) {
    ...
    const runs = repo.runsForCluster(db, tenantId, c.id, current);
  ...
  // -------------------------------------------------------- per-family summary
  const familySummaries = summariseByFamily(db, tenantId, brandId, current, clusters);
...
function summariseByFamily(db: DB, tenantId: string, brandId: string, windowLabel: string, clusters: repo.Row[]) {
  ...
  for (const [family, list] of byFamily) {
    ...
    const runs = runsInClusters(db, tenantId, brandId, list.map((c) => c.id), windowLabel);
    let defectRuns = 0;
    for (const r of runs) {
      const obs = repo.observedForRun(db, tenantId, r.id);
...
function buildDefectHeadline(providers: string[], verdict: string, m: Measurement, predicate: string): string {
  ...
  const verb = verdict === 'STALE' ? 'repeat an out-of-date account of' : 'describe';
  const qualifier = verdict === 'STALE' ? '' : ' incorrectly';
  ...
  return `${who} ${verb}${qualifier} ${subject} in ${rate}.`;
}
